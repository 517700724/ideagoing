...
function godetails(id){
	var dialog=_jM.getParam("dialog");
	var inv_id = _jM.getParentParam('inv_id');//邀请id
	if(_jM.validate.isNotEmpty(dialog) && dialog){
		window.parent._rd.showCategory(id);
		window.parent._jM.dialog.hiddenModeDialog("categoryDialog");
	}else{
		if(window.parent){
